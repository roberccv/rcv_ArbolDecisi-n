...
public class Modelo {
    ...
    public void aprenderModelo()
    {
        try {
            ...
            // train
            Instances inst = leerInstancias("./training_data/phishing.arff");
            cls.buildClassifier(inst);

            // serialize model
            ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream("./models/objetoRFPhishing.model"));
            oos.writeObject(cls);
            oos.flush();
            oos.close();
        } catch (Exception ex) {
    ...
    public String aplicarModelo() {
        try{
            ...
            Classifier clasificador  = (Classifier) weka.core.SerializationHelper.read("./models/objetoRFPhishing.model");
